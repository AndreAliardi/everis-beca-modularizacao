...
class MainActivity : AppCompatActivity() {
    ...
    private fun setListener() {
        btLogin.setOnClickListener {
            val loginIntent = Intent(this, br.everis.login.ui.LoginActivity::class.java)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == LOGIN_REQUEST_CODE && resultCode == br.everis.login.constants.LoginConstants.LOGIN_RESULT_CODE) {
            ...
            if (loginOk == true) {
                ...
                onError(
                    data?.extras?.getString(br.everis.login.constants.LoginConstants.EXTRA_ERROR_KEY) ?: UNKNOWN_LOGIN_ERROR
                )
            }
        }
    }
